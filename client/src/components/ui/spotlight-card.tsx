...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  useEffect(() => {
    const syncPointer = (e: PointerEvent) => {
      ...
      if (cardRef.current) {
        ...
        // Calculate position relative to the viewport for the "fixed" background effect
        // but the spotlight looks better if it's aware of the card boundaries
  ...
  const getInlineStyles = () => {
    const baseStyles: React.CSSProperties & { [key: string]: string | number } = {
      ...
      '--spread': spread,
      '--radius': '32',
      '--border': '2', // Increased for better visibility
      '--backdrop': 'rgba(13, 13, 13, 0.4)',
      '--backup-border': 'rgba(255, 255, 255, 0.1)',
      '--size': '450', // Slightly larger
      '--outer': '1',
      ...
      backgroundOrigin: 'border-box', // Added
