// Adapted from ibelick/motion-primitives — uses framer-motion instead of motion/react
import {
  Children,
  ReactNode,
  createContext,
...
function CarouselContent({ children, className, transition }: CarouselContentProps) {
  ...
  return (
    <motion.div
      ...
      className={cn('flex items-start', !disableDrag && 'cursor-grab active:cursor-grabbing', className)}
      ref={containerRef}
    >
      {children}
    </motion.div>
...
export { Carousel, CarouselContent, CarouselNavigation, CarouselItem, useCarousel };
