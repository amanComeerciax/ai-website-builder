...
<section id="contact" style="padding:120px 32px;background:${theme.bg};">
  <div style="max-width:1200px;margin:0 auto;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1.4fr;gap:60px;align-items:start;">
      ...
      <form id="contact-form" style="display:flex;flex-direction:column;gap:16px;" onsubmit="event.preventDefault();var f=this,btn=f.querySelector('button[type=submit]');btn.disabled=true;btn.innerHTML='Sending...';var d={name:f.querySelector('[name=cname]').value,email:f.querySelector('[name=cemail]').value,message:f.querySelector('[name=cmessage]').value,projectId:window.__PROJECT_ID||''};fetch('/api/contact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)}).then(function(r){return r.json()}).then(function(){f.innerHTML='<div style=\\'text-align:center;padding:40px;color:${theme.accent};font-family:sans-serif;font-weight:600;font-size:1.1rem\\'>✓ Message sent! We\\'ll be in touch soon.</div>'}).catch(function(){f.innerHTML='<div style=\\'text-align:center;padding:40px;color:${theme.accent};font-family:sans-serif;font-weight:600;font-size:1.1rem\\'>✓ Message sent! We\\'ll be in touch soon.</div>'})">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          ...
          <div>
            ...
            <input type="email" name="cemail" placeholder="you@example.com" required style="width:100%;padding:13px 16px;border-radius:12px;border:1px solid ${inputBorder};background:${inputBg};color:${theme.text};font-family:'${theme.fontBody}',sans-serif;font-size:0.93rem;outline:none;box-sizing:border-box;transition:border-color 0.2s;" onfocus="this.style.borderColor='${theme.accent}'" onblur="this.style.borderColor='${inputBorder}'">
          </div>
        </div>
        <div>
