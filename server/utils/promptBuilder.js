...
function buildTrackAPrompt(sitePlan) {
  ...
  const systemPrompt = [
    ...
    '7. IMAGES — AUTONOMOUS CONTEXT-AWARE SYSTEM:',
    '   For EVERY <img> tag, think about what the image should ACTUALLY depict based on the page content.',
    '   Write a short visual description (3-8 words) and use this URL pattern:',
    '   https://image.pollinations.ai/prompt/{URL-ENCODED-DESCRIPTION}?width={W}&height={H}&nologo=true',
    '   Sizes: Hero=width=1200&height=800, Cards=width=800&height=600, Thumbnails=width=400&height=400',
    '   Example for a chai shop menu card "Masala Chai":',
    ...
    '   RULES: Each image MUST have a UNIQUE description matching its card/section. NEVER repeat URLs.',
    '   BANNED: source.unsplash.com (DEAD), placehold.co, empty src=""',
  ...
  const userMessage = [
    ...
    '- Gallery: include 6-8 image cards in a responsive grid',
    '- NEVER create a section with only a heading and blank space below it',
    '',
    'IMAGE RULES (CRITICAL):',
    '- For EVERY image, think: "What should this image actually show for THIS specific card/section?"',
    '- Write a short, specific description and use: https://image.pollinations.ai/prompt/{description}?width=800&height=600&nologo=true',
    '- Hero images: use width=1200&height=800. Card images: use width=800&height=600.',
    '- Each image MUST have a UNIQUE description. NEVER use the same URL twice.',
    '- NEVER use source.unsplash.com, placehold.co, or leave src="" empty.',
    '- Use CSS gradients as decorative backgrounds where appropriate.',
    '',
